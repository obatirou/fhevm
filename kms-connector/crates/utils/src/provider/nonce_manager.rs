...
pub struct ZamaNonceManager {
    accounts: Arc<DashMap<Address, Arc<Mutex<AccountState>>>>,
}
...
impl ZamaNonceManager {
    ...
    /// Clears the account state of the nonce manager.
    ///
    /// Allow the nonce manager to sync with the blockchain RPC node at next `get_next_nonce` call.
    pub fn clear(&self) {
        self.accounts.clear();
    }
        ...
        N: Network,
    {
        ...
        let nonce_to_use =
            if let Some(available_nonce) = state.available_nonces.iter().next().copied() {
    ...
    where
        ...
        N: Network,
    {
        // Use the atomic `get` API from DashMap.
        ...
        let account_mutex = self.accounts.entry(address).or_insert_with(|| {
            // debug!(%address, nonce = initial_nonce, "Atomically initializing new account state");
