...
contract ProtocolPayment is IProtocolPayment, UUPSUpgradeableEmptyProxy, GatewayOwnable {
    ...
    /**
     * @notice See {IProtocolPayment-sendFeesTo}.
     */
    function sendFeesToBurner() external virtual onlyGatewayOwner {
        // Get the total amount of fees aggregated by the ProtocolPayment contract
        uint256 balance = ZAMA_OFT.balanceOf(address(this));

        // Send all fees to the burner contract
        (bool success, ) = FEES_SENDER_TO_BURNER_ADDRESS.call(
            abi.encodeWithSignature("sendFeesToBurner(uint256)", balance)
        );
        if (!success) {
            revert SendFeesToBurnerFailed();
        }
        emit FeesSentToBurner(balance);
    }
